...
namespace chip8 {
    class Cpu_test {
        public:
            ...
            void testInit() {
                std::vector<unsigned char> kPresets = {
                ...
                for (size_t i = 0; i < 16; ++i) {
                    auto actual_font = ram.load(i * Cpu::kFontHeight, Cpu::kFontHeight);
                    for (size_t j = 0; j < Cpu::kFontHeight; ++j) {
                        assert(actual_font[j] == kPresets[i * Cpu::kFontHeight + j]);
                    }
                }
                assert(ram.data_[0x200] == 0);
            }
            ...
            void testShlVxVy() {
                ...
                ins.regs_.v[0xa] = value;
                ins.execute(info);
                assert(ins.regs_.v[0xa] == static_cast<uint8_t>(value * 2));
                assert(ins.regs_.v[0xf] == 1);
